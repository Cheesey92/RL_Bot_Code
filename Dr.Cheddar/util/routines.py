...
class dodge(Routine):
    ...
    def run(self, agent):
        ...
        if elapsed < 0.15:
            ...
            agent.controller.jump = True
            agent.controller.pitch = self.pitch * 0.8
            agent.controller.yaw = self.yaw *0.5
        else:
            agent.set_intent(recovery())
...
class dodge_kickoff(Routine):
    # won't miss the boost on the slight-offcenter kickoffs haha
    def run(self, agent):
        target = agent.ball.location + Vector3(0, 200*side(agent.team), 0)
        local_target = agent.me.local(target - agent.me.location)
        defaultPD(agent, local_target)
        ...
        if local_target.magnitude() < 650:
            # flip towards opponent goal
            agent.set_intent(
                dodge(agent.me.local(agent.foe_goal.location - agent.me.location)))
